...
def _attempt(driver, search_url, search_link_text):
    ...
    # checkout
    driver.find_element_by_css_selector(".item-cell:nth-child(1) .checkout-step-action-done").click() # step 1
    driver.find_element_by_css_selector(".item-cell:nth-child(2) .checkout-step-action-done").click() # step 2
    cvv2 = driver.find_element_by_css_selector(".mask-cvv-4")
    ActionChains(driver).click(cvv2).send_keys(settings.CVV2).perform()
    driver.find_element_by_css_selector(".item-cell:nth-child(3) .checkout-step-action > .btn").click() # step 3
    # TODO auto checkout? YOLO


def attempt_purchase(search_url="https://www.newegg.com/p/pl?d=5900x", search_link_text="AMD RYZEN 9 5900X"):
